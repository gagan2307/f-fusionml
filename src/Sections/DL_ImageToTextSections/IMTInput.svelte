...
<script>
    ...
    async function analyzeImage() {
        ...
        try {
            ...
            const response = await axios.post('/api/dl-image-to-text', formData);

            if (response.status !== 200) {
                throw new Error('Network response was not ok');
            }

            captionResult = response.data.caption || 'No caption received.';
        } catch (error) {
            console.error('Error:', error);
            captionResult = 'Error analyzing image.';
        } finally {
            isLoading = false;
...
<div class="flex flex-col items-center justify-center gap-6 p-4">
    ...
    <div class="flex w-full max-w-6xl flex-col gap-8 md:flex-row">
        ...
        <div class="flex w-full flex-col rounded-lg bg-white/80 p-6 shadow-lg md:w-1/3">
            <h2 class="mb-4 text-center text-2xl font-bold">Caption Result</h2>
            <textarea
                bind:value={captionResult}
